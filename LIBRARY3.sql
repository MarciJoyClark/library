...
CREATE TABLE book (
	...
	Publisher VARCHAR(50) CONSTRAINT pub_name FOREIGN KEY REFERENCES publisher_info(pub_name) ON UPDATE CASCADE ON DELETE CASCADE,
);
...
INSERT INTO book
	...
	('Madame Tussaud: A Life in Wax', 'Harper-Collins'),
	...
	('Technology Basics Dictionary: Tech and Computers Simplified', 'Jack Stanley'),
...
CREATE TABLE author (
	a_book_ID INT NOT NULL CONSTRAINT a_book_id FOREIGN KEY REFERENCES book(book_id) ON UPDATE CASCADE ON DELETE CASCADE,
...
CREATE TABLE borrower (
	...
	brw_addr NVARCHAR(100),
	brw_phone NVARCHAR(20)
);
...
CREATE TABLE loans (
	l_book_ID INT NOT NULL CONSTRAINT l_book_ID FOREIGN KEY REFERENCES book(book_ID) ON UPDATE CASCADE ON DELETE CASCADE,
	l_branch_ID INT NOT NULL CONSTRAINT l_branchID FOREIGN KEY REFERENCES branch(branch_ID) ON UPDATE CASCADE ON DELETE CASCADE,
	l_card_no NVARCHAR(8) NOT NULL CONSTRAINT l_card_no FOREIGN KEY REFERENCES borrower(card_no)  ON UPDATE CASCADE ON DELETE CASCADE,
...
INSERT INTO loans
	(l_book_ID, l_branch_ID, l_card_no, DATE_OUT, DATE_DUE)
		VALUES
...
CREATE PROC dbo.uspsharpstown_duebooks
	AS
		SELECT DISTINCT full_NAME, brw_addr, book.book_title AS title
		FROM borrower, loans, book, branch
			WHERE loans.date_due = '01-02-2018'
			  AND
			  card_no = loans.l_card_no
			  AND
			  loans.l_book_id = book.book_id
			  AND
			  branch.branch_name = 'sharpstown'
GO
...
EXEC dbo.uspsharpstown_duebooks_plus5
